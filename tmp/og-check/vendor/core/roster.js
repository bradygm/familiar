 ...
 */
import { cardPredictedRecall, learningDifficulty } from './model.js';
const byName = (left, right) => left.last_name.localeCompare(right.last_name) || left.first_name.localeCompare(right.first_name);
/**
 ...
 */
export function sortRoster(cards, sort, now = Date.now()) {
    const sorted = cards.slice();
    switch (sort) {
        case 'first':
            return sorted.sort((left, right) => left.first_name.localeCompare(right.first_name) || left.last_name.localeCompare(right.last_name));
        case 'last':
            return sorted.sort(byName);
        case 'recall':
            return sorted.sort((left, right) => cardPredictedRecall(left, now) - cardPredictedRecall(right, now) || left.seen_count - right.seen_count || byName(left, right));
        case 'strength':
            return sorted.sort((left, right) => left.mastery - right.mastery || left.seen_count - right.seen_count || byName(left, right));
        case 'difficulty':
            return sorted.sort((left, right) => learningDifficulty(right) - learningDifficulty(left) || right.wrong_count - left.wrong_count || byName(left, right));
    }
}
