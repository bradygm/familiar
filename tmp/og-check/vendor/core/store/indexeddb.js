...
const byName = (left, right) => String(left.first_name).localeCompare(right.first_name) || String(left.last_name).localeCompare(right.last_name);
export class IndexedDbStore {
    ...
    close() {
        this.database?.close();
        this.database = null;
        for (const url of this.portraits.values())
            URL.revokeObjectURL(url);
        this.portraits.clear();
    }
    async readAll(name, index, key) {
    ...
    async cardsOf(courseId, reviewed) {
    ...
    async getCardHistory(_courseId, cardId) {
     ...
     */
    async absorbRoster(courseId, file, status, courseToCreate) {
        ...
        const known = new Map(existingCards.map((card) => [JSON.stringify([String(card.first_name).trim(), String(card.last_name).trim()]), card]));
        const db = await this.db();
    ...
    async approveCandidate(_courseId, cardId) {
        const db = await this.db();
        const transaction = db.transaction('cards', 'readwrite');
        const cards = transaction.objectStore('cards');
        const card = await wrap(cards.get(cardId));
        if (!card)
            throw new Error('No such candidate awaiting review');
        cards.put({ ...card, reviewed: 1 });
        await done(transaction);
    }
    async rejectCandidate(_courseId, cardId) {
        const card = await this.get('cards', cardId);
        if (!card || card.reviewed)
            throw new Error('No such candidate awaiting review');
        await this.deleteCard(cardId);
    }
    async removeCard(_courseId, cardId) {
        const card = await this.get('cards', cardId);
        if (!card)
            throw new Error('That person is not in this course');
        await this.deleteCard(cardId);
    }
    ...
    async deleteCard(cardId) {
        const card = await this.get('cards', cardId);
        const reviews = await this.readAll('reviews', 'card_id', cardId);
        const others = (await this.readAll('cards')).filter((other) => other.id !== cardId && other.image_path && other.image_path === card?.image_path);
        const db = await this.db();
        ...
        for (const review of reviews)
            transaction.objectStore('reviews').delete(review.id);
    ...
    async resetCourseProgress(courseId, confirmTitle) {
        ...
        for (const card of cards)
            transaction.objectStore('progress').put({ card_id: card.id, ...FRESH_PROGRESS });
    ...
    async deleteCourse(courseId, confirmTitle) {
        ...
        const keep = new Set((await this.readAll('cards'))
    ...
    async startSession(courseId, mode, cardIds) {
        const available = new Set((await this.readAll('cards', 'course_id', courseId)).filter((card) => card.reviewed).map((card) => card.id));
        if (available.size === 0)
            throw new Error('Approve or add cards before starting a session');
    ...
    async exportBundle(courseId, options = {}) {
    ...
    async importBundle(source) {
        ...
        if (!manifestBytes)
            throw new Error('That zip is not a Familiar backup.');
        ...
        const clashes = manifest.courses.filter((course) => existing.has(course.id)).map((course) => course.id);
        if (clashes.length) {
        ...
        for (const [name, bytes] of files) {
            ...
            transaction.objectStore('assets').put({ path, blob: new Blob([bytes], { type: 'image/jpeg' }) });
            counts.assets += 1;
        }
        await done(transaction);
        return counts;
