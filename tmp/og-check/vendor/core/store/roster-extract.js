 ...
 */
import { ROSTER_LAYOUT, readNameLine } from '../roster-text.js';
/**
...
const vendored = (path) => new URL(`../../${path}`, import.meta.url).href;
const VENDOR = {
...
async function createReader() {
    ...
    if (typeof createWorker !== 'function')
        throw new Error('The OCR library did not load correctly.');
...
function cropCanvas(source, left, top, right, bottom) {
 ...
 */
export async function extractRoster(file, onProgress = () => { }) {
    ...
    try {
        for (let pageNumber = 1; pageNumber <= pages; pageNumber += 1) {
            ...
            task.onContinue = (continueRendering) => setTimeout(continueRendering, 0);
            await task.promise;
...
async function readPage(reader, canvas) {
    ...
    for (const line of linesFrom(data)) {
        ...
        if (name)
            found.push({ ...name, portrait: await portraitFor(canvas, line.centre) });
    }
    ...
    if (found.length < ROSTER_LAYOUT.peoplePerPage) {
        ...
        for (const centre of ROSTER_LAYOUT.rowCentres) {
            ...
            const cell = cropCanvas(canvas, nameColumnLeft, middle - canvas.height * ROSTER_LAYOUT.rowHalfHeight, canvas.width, middle + canvas.height * ROSTER_LAYOUT.rowHalfHeight);
...
async function portraitFor(canvas, nameCentre) {
    const half = canvas.height * ROSTER_LAYOUT.portraitHalfHeight;
    const crop = cropCanvas(canvas, canvas.width * ROSTER_LAYOUT.portraitLeft, Math.max(0, nameCentre - half), canvas.width * ROSTER_LAYOUT.portraitRight, Math.min(canvas.height, nameCentre + half));
    return toJpeg(crop);
}
//# sourceMappingURL=roster-extract.js.map