 ...
 */
export function parseIsoInstant(value: string): Instant {
  const match = ISO_PATTERN.exec(value.trim());
  if (!match) throw new Error(`Not an ISO 8601 timestamp: ${JSON.stringify(value)}`);
...
export function toInstant(value: Instant | string | number | Date): Instant {
...
export function daysSince(
  ...
  now: Instant | string | number | Date,
): number {
  if (!timestamp) return NEVER_REVIEWED_DAYS;
