...
import { cardPredictedRecall, selectionScore, updateMemoryState, type CardProgress, type MemoryState, type ReviewResult } from './model.js';
import type { Random } from './selection.js';

/** Intervening reviews before a card in the cycle comes back. */
const MISS_GAP = 2;
const RECALL_GAPS = [3, 7] as const;
 ...
 */
export class ContinuousSession<T extends ContinuousCard> {
  private readonly cards: T[];
  private readonly random: Random;
  private readonly avoidRecent: number;
  private readonly cycles = new Map<string, CycleEntry>();
  private readonly recent: string[] = [];
  private reviewCount = 0;
  private recoveredCount = 0;
  private currentCard: T | null = null;
  private currentFromCycle = false;

  constructor(cards: readonly T[], options: ContinuousOptions = {}) {
    if (cards.length === 0) throw new Error('A continuous session needs at least one card.');
  ...
  next(now: string | number | Date = Date.now()): T {
   ...
   */
  record(result: ReviewResult, at: string | number | Date = Date.now()): { card: T; memory: MemoryState } {
    const card = this.currentCard;
    if (!card) throw new Error('Call next() before recording a result.');
    ...
    while (this.recent.length > this.avoidRecent) this.recent.shift();
  ...
  private byId(id: string | null): T | null {
    if (id === null) return null;
    return this.cards.find((card) => card.id === id) ?? null;
  }

  private highestScoring(now: string | number | Date): T {
    const avoid = new Set(this.recent);
    let best: T | null = null;
    let bestScore = -Infinity;
    for (const card of this.cards) {
      if (avoid.has(card.id)) continue;
