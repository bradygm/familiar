...
import { cardPredictedRecall, selectionScore, type CardProgress } from './model.js';
 ...
 */
export function adaptiveCards<T extends CardProgress>(
  cards: readonly T[],
  limit: number,
  ...
  if (cards.length <= limit) return shuffled(cards, random);
