...
> **Note:** the screenshots below predate the browser build and the interface rework, so
> they no longer match the app. The link-preview card at `frontend/og.png` is generated by
> `tools/make-og-image.py` and is current.
