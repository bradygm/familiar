...
- [ ] Retake the README screenshots. The current ones predate the browser build and the
      interface rework, so they show the old `data/` import flow and a course page that no
      longer looks like that. The link-preview card is separate and is current — it is
      generated by `tools/make-og-image.py`.
